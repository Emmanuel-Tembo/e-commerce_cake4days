  ...
  <div :class="`fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-xl transform transition-transform duration-500 ease-in-out z-[100] ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`">
    <div class="flex flex-col h-full">
      <div class="flex items-center justify-between p-6 border-b border-gray-200">
        <h2 class="text-2xl font-bold text-gray-800">Your Cart ({{ cartItems.length }})</h2>
      ...
      <div class="p-6 bg-gray-50 border-t border-gray-200">
        ...
        <div class="grid gap-2">
          ...
          <router-link to="/cart" @click="toggleCart" class="w-full bg-pink-800 text-white px-4 py-3 rounded-full font-semibold hover:bg-pink-900 text-center">
            Checkout
          </router-link>
        </div>
      </div>
